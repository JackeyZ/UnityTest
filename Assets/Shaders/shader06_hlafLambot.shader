...
Shader "MyShader/shader06 half lambot"{
	Properties{
		...
		Pass{
			CGPROGRAM
				...
				v2f vert(a2v v){ 
					...
					f.worldPos=mul(unity_ObjectToWorld,v.vertex).xyz; //顶点坐标从模型空间转化到
					return f;
				}

				fixed4 frag(v2f f) : SV_Target{
					fixed3 ambient = UNITY_LIGHTMODEL_AMBIENT.rgb; //获取环境光
					fixed3 lightDir = normalize(_WorldSpaceLightPos0.xyz);//获取光照方向
